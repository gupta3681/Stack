...
def test_creating_a_task_for_a_date_persists_the_stack(auth_client: TestClient):
    auth_client.post(
        "/tasks", json={"name": "first", "stack_date": "2026-06-01"}
    )
    body = auth_client.get("/stacks/2026-06-01").json()
    assert body["id"] is not None
    assert len(body["tasks"]) == 1
    assert body["tasks"][0]["name"] == "first"
...
def test_overdue_respects_client_today_param(auth_client: TestClient):
    ...
    long_ago = "2026-01-01"
    auth_client.post("/tasks", json={"name": "old task", "stack_date": long_ago})
...
def test_counts_today_tomorrow_topic_stacks(auth_client: TestClient):
    ...
    auth_client.post("/tasks", json={"name": "a", "stack_date": today})
    t = auth_client.post(
        "/tasks", json={"name": "b", "stack_date": today}
    ).json()
    auth_client.patch(f"/tasks/{t['id']}", json={"status": "done"})
    # 3 tomorrow
    for title in ("x", "y", "z"):
        auth_client.post("/tasks", json={"name": title, "stack_date": tomorrow})
    # 2 topic stacks
    auth_client.post(
...
def test_counts_respects_user_scope(
    ...
    today = date.today().isoformat()
    auth_client.post("/tasks", json={"name": "mine", "stack_date": today})
    second_client.post("/tasks", json={"name": "theirs", "stack_date": today})
    second_client.post("/tasks", json={"name": "theirs2", "stack_date": today})
    assert (
...
def test_overdue_excludes_done_tasks(auth_client: TestClient):
    long_ago = "2026-01-01"
    t = auth_client.post(
        "/tasks", json={"name": "done already", "stack_date": long_ago}
    ).json()
    auth_client.patch(f"/tasks/{t['id']}", json={"status": "done"})
    today = date.today().isoformat()
...
def test_overdue_excludes_cancelled_tasks(auth_client: TestClient):
    ...
    long_ago = "2026-01-01"
    t = auth_client.post(
        "/tasks", json={"name": "not gonna happen", "stack_date": long_ago}
    ).json()
