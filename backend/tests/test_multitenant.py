...
def test_users_have_independent_daily_stacks(
    ...
    auth_client.post(
        "/tasks", json={"title": "aryan's task", "stack_date": "2026-06-01"}
    )
    second_client.post(
        "/tasks", json={"title": "other's task", "stack_date": "2026-06-01"}
    )
    a = auth_client.get("/stacks/2026-06-01").json()
    b = second_client.get("/stacks/2026-06-01").json()
    assert [t["title"] for t in a["tasks"]] == ["aryan's task"]
    assert [t["title"] for t in b["tasks"]] == ["other's task"]
...
def test_user_cannot_modify_other_users_task(
    ...
    t = auth_client.post(
        "/tasks", json={"title": "private", "stack_date": "2026-06-01"}
    ).json()
    # Other user trying to mutate by ID — all should 404
    assert second_client.get(f"/tasks/{t['id']}").status_code == 404
    assert (
        second_client.patch(f"/tasks/{t['id']}", json={"title": "hijack"}).status_code
        == 404
    )
...
def test_user_cannot_inject_foreign_task_into_their_reorder(
    ...
    foreign = auth_client.post(
        "/tasks", json={"title": "aryan", "stack_date": "2026-06-01"}
    ).json()
    mine = second_client.post(
        "/tasks", json={"title": "mine", "stack_date": "2026-06-01"}
    ).json()
    ...
    assert auth_client.get(f"/tasks/{foreign['id']}").json()["title"] == "aryan"
...
def test_overdue_only_returns_own_tasks(
    ...
    long_ago = "2026-01-01"
    auth_client.post("/tasks", json={"title": "aryan-old", "stack_date": long_ago})
    second_client.post("/tasks", json={"title": "other-old", "stack_date": long_ago})
    a = auth_client.get("/stacks/overdue?today=2026-12-31").json()
    b = second_client.get("/stacks/overdue?today=2026-12-31").json()
    assert [t["title"] for t in a] == ["aryan-old"]
    assert [t["title"] for t in b] == ["other-old"]
